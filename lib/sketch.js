 ...
 * @type {String}
 */
sketch.toolBinary = '/Applications/Sketch.app/Contents/Resources/sketchtool/bin/sketchtool';

/**
 * Migrate Binary Path
 * @type {String}
 */
sketch.migrateBinary = '/Applications/Sketch.app/Contents/Resources/sketchtool/bin/sketchmigrate';
...
sketch.dump = function (file, callback) {

  var self = this;
  var resp = '';

  this.process = spawn(this.toolBinary, ['dump', file]);

  this.process.stdout.on('data', function (data) {
    resp += data.toString();
  });

  this.process.stdout.on('end', function(data) {
    self._resetProcess();
    callback(resp);
    return true;
  });
